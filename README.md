...
1. Set up a dictionary called `variables` containing the following five key:value pairs: `'B02001_001E':'pop_total'`,`'B02001_002E':'pop_white'`,`'B25003_001E':'housing_total'`,`'B25003_002E':'housing_owned'`,`'B25003_003E':'housing_rental'`. It will be a convenient way to rename the variables later on.
...
1. Set `pop` to the result of calling `.replace()` on `pop` using two arguments: `"-666666666"` and `np.nan`). That value is used by the API to indicate missing data, and it has nine 6s.
...
1. Set variable `keep_cols` to a list consisting of the strings `'GEOID'`,`'COUNTYFP'`,`'geometry'`. Then set `bgs` to the result of using `keep_cols` to select the desired columns from `bgs`.
...
1. Create a column in `plot_data` called `'Distance'` that is equal to the result of calling `.astype(int).astype(str)` on the `'radius'` column of `plot_data`. That makes set of string labels for the distances.

1. Set the `'Distance'` column just created to the value of calling the `.replace()` method on it with arguments `'9999'` and `'>3200'`. That will improve the label for the area beyond the outer ring.
...
1. Now set `fg` equal to the result of calling `sns.relplot()` with the following fairly long list of arguments: `data=plot_data`, `x='pct_poc'`, `y='pct_rental'`, `hue='Distance'`, `size='Population'`, `sizes=(10,200)`,
